...
class MissionControlNode(Node):
    def __init__(self):
        ...
        # State Machine: IDLE, NAV_TO_PICKUP, AT_PICKUP, NAV_TO_DROPOFF
        self.mission_phase = "IDLE"
        self.current_order = None
        self.order_queue = []
        self.priority_queue = []
        self.wait_timer = None # Timer for waiting at locations
    ...
    def load_locations(self):
        ...
        # ADD YOUR CUSTOM LOCATIONS AND COORDINATES HERE
        self.locations = {
    ...
    def load_route(self):
        # ... (load_route function remains the same as before) ...
    ...
    def priority_order_callback(self, msg):
        # ... (priority_order_callback function remains the same as before) ...
    ...
    def nav_status_callback(self, msg: String):
        ...
        nav_status = msg.data
        if self.mission_phase not in ["NAV_TO_PICKUP", "NAV_TO_DROPOFF"]:
            return

        if nav_status == "SUCCEEDED_AT_POSE":
            if self.mission_phase == "NAV_TO_PICKUP":
                self.mission_phase = "AT_PICKUP"
                self.get_logger().info("Arrived at pickup. Waiting for 5 seconds...")
                # Use a non-blocking timer instead of time.sleep()
                ...
                self.current_order = None
                self.mission_phase = "IDLE" # Ready for next order
    ...
    def pickup_complete_callback(self):
        """Called by the timer after waiting at the pickup location."""
        self.wait_timer.cancel() # The timer is one-shot
    ...
    def send_goal(self, x, y, yaw_degrees):
        # ... (send_goal function remains the same as before) ...
